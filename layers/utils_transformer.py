...
from layers.point_transfomer import PointNetSetAbstractionMsg_GridPool_PTv1, PointNetFeaturePropagationFast
...
class get_model(nn.Module):
    def __init__(self):
        ...
        self.sa1 = PointNetSetAbstractionMsg_GridPool_PTv1(1024, [0.05], [5], 59, [[128]],pt_out_channel=128)
        self.sa2 = PointNetSetAbstractionMsg_GridPool_PTv1(256, [0.1], [4], 128, [[192]],pt_out_channel=192)
        self.sa3 = PointNetSetAbstractionMsg_GridPool_PTv1(64, [0.2], [4], 192, [[256]],pt_out_channel=256)
        # self.sa4 = PointNetSetAbstractionMsg(16, [0.4, 0.8], [4, 8], 256+256, [[256, 256, 512], [256, 384, 512]])
        # self.fp4 = PointNetFeaturePropagation(512+512+256+256, [256, 256])
        self.fp3 = PointNetFeaturePropagationFast(256+192, [256, 256])
    ...
    def forward(self, N_Gaussians,xyz):
        ...
        l1_xyz, l1_points = self.sa1(l0_xyz, l0_points,N_Gaussians//4)
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points,N_Gaussians//8)
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points,N_Gaussians//12)
        ...
        x_dc = self.feac_dc_dec(l0_points)
        x_ac = self.feac_ac_dec(l0_points)
        # x = F.log_softmax(x, dim=1)
        # x = l0_points[:,3:,:]
        x = torch.cat((x_dc.permute(0, 2, 1),x_ac.permute(0,2,1)),dim=-1)
        return x