...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_p = nn.Sequential(nn.Linear(3, 3), nn.BatchNorm2d(3), nn.LeakyReLU(inplace=True), nn.Linear(3, out_planes))
        self.linear_w = nn.Sequential(nn.BatchNorm2d(mid_planes), nn.LeakyReLU(inplace=True),
                                    nn.Linear(mid_planes, mid_planes // share_planes),
                                    nn.BatchNorm2d(mid_planes // share_planes), nn.LeakyReLU(inplace=True),
    ...
    def forward(self, all_feats) -> torch.Tensor:
        ...
        feats_k = queryandgroup(self.nsample, position, position, feats_k, None, use_xyz=True)  # (b,m, nsample, 3+c)
        feats_v = queryandgroup(self.nsample, position, position, feats_v, None, use_xyz=False)  # (b,m, nsample, c)
        if feats_k == None or feats_v == None:
            return None
        p_r, feats_k = feats_k[:, :, :, 0:3], feats_k[:, :, :, 3:]
        for i, layer in enumerate(self.linear_p): p_r = layer(p_r.permute(0, 3, 2, 1).contiguous()).permute(0, 3, 2, 1).contiguous() if i == 1 else layer(p_r)    # (b,m, nsample, c)
        ...
        for i, layer in enumerate(self.linear_w): w = layer(w.permute(0, 3, 2, 1).contiguous()).permute(0, 3, 2, 1).contiguous() if i % 3 == 0 else layer(w)
...
class PointTransformerBlock(nn.Module):
    ...
    def forward(self, all_feats):
        ...
        feats = self.relu(self.bn1(self.linear1(feats).transpose(1,2).contiguous()).transpose(1,2).contiguous())
        feats = self.transformer2([position.contiguous(), feats.contiguous()])
        if feats == None:
            return None
        feats = self.relu(self.bn2(feats.transpose(1,2).contiguous()).transpose(1,2).contiguous())
        feats = self.bn3(self.linear3(feats).transpose(1,2).contiguous()).transpose(1,2).contiguous()
        feats += identity
