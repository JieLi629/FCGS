...
class get_model(nn.Module):
    ...
    def forward(self, N_Gaussians,g_coords_feac_fusion):
        ...
        mask_sig = self.Encoder_mask(torch.cat((g_feac,g_geo),dim=-1))
        mask_fea = mask_sig + ((mask_sig > 0.01).float() - mask_sig).detach()# note the role of detach(), connect the gradient
        # mask_fea = _opacity < 0
        mask_bool = mask_fea.to(torch.bool).squeeze(-1)
