...
from layers.pointnetpp_utils_mask_CTX_argu_m0_geo import Encoder, Decoder, PostProcess, Sensity_Mask, hyper_decoder_geo,hyper_decoder_m0,hyper_decoder_m1,hyper_encoder_geo,hyper_encoder_m0,hyper_encoder_m1,build_static_gs_mask
from layers.pointnetpp_utils_mask_CTX_argu_m0_geo import Spatial_Context_Model,Channel_Context_Model_M1,Channel_Context_Model_M0,Spatial_Cxt_PostProcess, normalize_xyz,get_resolution_list
...
from layers.entropy_model_argu_m0_geo import Factorized_Gaussian_Model
...
class get_model(nn.Module):
    def __init__(self,
        ...
        self.Encoder_hyper_geo = hyper_encoder_geo(8, 16)
        self.Decoder_hyper_geo = hyper_decoder_geo(16, 24)
        self.Factorize_model_m1 = Factorized_Gaussian_Model(32)
        self.Factorize_model_m0 = Factorized_Gaussian_Model(24)
        self.Gaussian_Entropy_Model = GaussianConditional(None,scale_bound=1e-6)
        self.Factorize_model_geo = Factorized_Gaussian_Model(16)
    ...
    def forward(self, N_Gaussians,g_coords_feac_fusion):
        ...
        choose_idx = torch.rand_like(coords.squeeze(0)[:, 0]) <= 0.05
        ...
        feac_m1_logits = -torch.sum(torch.log2(likelihood_feac_m1.clamp(min=1e-9)),dim=-1).unsqueeze(-1)
        ...
        feac_m0_logits = -torch.sum(torch.log2(likelihood_feac_m0.clamp(min=1e-9)),dim=-1).unsqueeze(-1)
        ...
        # geo_scale_quant = geo_scale + torch.empty_like(geo_scale).uniform_(-0.5,0.5)
        ...
        logits_geo = -torch.sum(torch.log2(likelihood_geo.clamp(min=1e-9)),dim=-1).unsqueeze(-1)
