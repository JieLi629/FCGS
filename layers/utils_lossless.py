...
from layers.pointnetpp_utils_lossless import PointNetSetAbstractionMsg_GridPool, PointNetFeaturePropagationFast, PointNetSetAbstraction,PointNetFeaturePropagation
...
def render_train(g_xyz,g_geo,camara_list,gaussians_feac_dec,pipline,C,H,W):
    ...
    g_opacity,g_scaling,g_rotation = torch.split(g_geo[0,:,:],split_size_or_sections=[1,3,4],dim=-1)
    f_dc,f_rst = torch.split(gaussians_feac_dec[0,:,:],split_size_or_sections=[3,45],dim=-1)
...
class get_model(nn.Module):
    def __init__(self):
        ...
        self.flat_layer_enc = PointNetSetAbstraction(56, [64, 96, 128])
        self.down_layer1 = PointNetSetAbstractionMsg_GridPool([4], 128, [128, 192, 256])
        self.down_layer2 = PointNetSetAbstractionMsg_GridPool([4], 256, [256, 384, 512])
        # self.sa4 = PointNetSetAbstractionMsg(16, [0.4, 0.8], [4, 8], 256+256, [[256, 256, 512], [256, 384, 512]])
        # self.fp4 = PointNetFeaturePropagation(512+512+256+256, [256, 256])
        self.flat_layer_dec = PointNetFeaturePropagation(512, [512, 512])
        self.up_layer1 = PointNetFeaturePropagationFast(512+256, [512, 256])
        self.up_layer2 = PointNetFeaturePropagationFast(256+128, [256, 128])
    ...
    def forward(self, N_Gaussians,points):
        l0_points = points
        l0_xyz = points[:,:,:3]
        l0_attri = points[:,:,3:]
        ...
        l1_xyz, l1_points = self.flat_layer_enc(l0_xyz, l0_attri)
        if l1_xyz == None and l1_points == None:
            return None
        l2_xyz, l2_points = self.down_layer1(l1_xyz, l1_points,N_Gaussians//4)
        if l2_xyz == None and l2_points == None:
            return None
        l3_xyz, l3_points = self.down_layer2(l2_xyz, l2_points,N_Gaussians//16)
        ...
        l3_points = self.flat_layer_dec(l3_points)

        l2_points = self.up_layer1(l2_xyz, l3_xyz, l2_points, l3_points)
        l1_points = self.up_layer2(l1_xyz, l2_xyz, l1_points, l2_points)
        

        # x = F.leaky_relu(self.bn1(self.conv1(l0_points)))
        x_dc = self.feac_dc_dec(l1_points)
        x_ac = self.feac_ac_dec(l1_points)
