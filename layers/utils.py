...
class get_model(nn.Module):
    def __init__(self):
        ...
        self.Q_fe = 0.001
        # self.sa1 = PointNetSetAbstractionMsg_GridPool(1024, [0.05], [3], 59, [[128, 128, 128]])
        ...
        ########################for centroid#####################
        self.sa1 = PointNetSetAbstractionMsg_GridPool(1024, [0.12], [24], 64, [[128,128,128]])
        self.sa2 = PointNetSetAbstractionMsg_GridPool(256, [0.24], [12], 128, [[128,192,256]])
        # self.sa3 = PointNetSetAbstractionMsg_GridPool(64, [0.6], [4], 256, [[256, 384, 512]])
        ...
        self.Q_l0 = nn.Parameter(torch.tensor(data=1.0))
        # self.Q_l1 = nn.Parameter(torch.tensor(data=1.0))
        # self.Q_l2 = nn.Parameter(torch.tensor(data=1.0))
        # self.Q_l3 = nn.Parameter(torch.tensor(data=1.0))
    ...
    def forward(self, N_Gaussians,g_coords_feac_fusion):
        l0_points = g_coords_feac_fusion[:,3:,:]
        l0_xyz = g_coords_feac_fusion[:,:3,:]
        ...
        l1_xyz, l1_points = self.sa1(l0_xyz, raw_fea,int(N_Gaussians/3))
        ...
        if l2_xyz == None and l2_points == None:
            return None
        # l3_xyz, l3_points = self.sa3(l2_xyz, l2_points,N_Gaussians//18)
        ...
        Q_layer0 = self.Q_fe * (1 + torch.tanh(self.Q_l0))
        # Q_layer1 = self.Q_fe * (1 + torch.tanh(self.Q_l1)) * 3
        # Q_layer2 = self.Q_fe * (1 + torch.tanh(self.Q_l2)) * 9
        # Q_layer3 = self.Q_fe * (1 + torch.tanh(self.Q_l3)) * 27
        raw_fea = raw_fea + torch.empty_like(raw_fea).uniform_(-0.5,0.5) * Q_layer0
        l1_points = l1_points + torch.empty_like(l1_points).uniform_(-0.5,0.5) * Q_layer0
        l2_points = l2_points + torch.empty_like(l2_points).uniform_(-0.5,0.5) * Q_layer0
        # l3_points = l3_points + torch.empty_like(l3_points).uniform_(-0.5,0.5) * Q_layer0
