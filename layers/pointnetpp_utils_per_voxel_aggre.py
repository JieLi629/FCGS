...
from torch_scatter import scatter_max, scatter_mean, scatter_min, scatter_add, scatter_softmax
...
class PointNetSetAbstractionMsg_GridPool(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list, grid_size=None):
        ...
        for i in range(len(mlp_list)):
            ...
            last_channel = in_channel
            for out_channel in mlp_list[i]:
                convs.append(nn.Conv1d(last_channel, out_channel,kernel_size=1))
                bns.append(nn.BatchNorm1d(out_channel))
                last_channel = out_channel
            self.conv_blocks.append(convs)
            self.bn_blocks.append(bns)
        self.linear_p = nn.Sequential(nn.Conv1d(3, 3, 1), nn.BatchNorm1d(3), nn.LeakyReLU(inplace=True), nn.Conv1d(3, mlp_list[0][-1], 1))
    ...
    def grid_pool_downsample(self, xyz, target_npoint, return_counts=True):
        ...
        #         mask = (new_inverse == -1)
        #         if mask.any():
        #             num_unselected = int(mask.sum().item())
        #             new_inverse[mask] = torch.randint(0, int(target_npoint), (num_unselected,), device=device)
                    
        #     elif current_n < target_npoint:
             
        #         deficit = target_npoint - current_n
        #         if current_n > 0 and deficit <= 0.005 * target_npoint:
        ...
        #             new_inverse = inverse_b  # [N], 值范围 0~current_n-1 ⊂ 0~target_npoint-1
    ...
    def forward(self, xyz, points, self_define_samples=None):
        ...
        for i, radius in enumerate(self.radius_list):
            for b in range(B):
                centers = new_xyz[b][inverse[b]]
                grouped_xyz = xyz[b] - centers
                grouped_xyz_list.append(grouped_xyz)
            grouped_xyz = torch.stack(grouped_xyz_list,dim=0)
            
            grouped_points = points.permute(0,2,1)
            for j in range(len(self.conv_blocks[i])):
                conv = self.conv_blocks[i][j]
                bn = self.bn_blocks[i][j]
                grouped_points = F.leaky_relu(bn(conv(grouped_points)))
            grouped_points = grouped_points.permute(0,2,1)
            xyz_trans = self.linear_p(grouped_xyz.permute(0,2,1)).permute(0,2,1)
            inverse = (inverse[0].unsqueeze(-1)).unsqueeze(0)
            weight_position = scatter_softmax(xyz_trans, inverse, dim=1)
            new_points = grouped_points * weight_position
            new_points = scatter_add(new_points,inverse,dim=1)
            new_points_list.append(new_points)
        
        new_xyz = new_xyz.permute(0, 2, 1)  # [B, 3, S]
        new_points_concat = torch.cat(new_points_list, dim=1).permute(0,2,1)  # [B, D'_total, S]
        return new_xyz, new_points_concat
...
class PointNetFeaturePropagationFast(nn.Module):
    def __init__(self, in_channel, mlp, po_out_channel, k=3, grid_size = None):
        super().__init__()
        self.k = k
        self.grid_size = grid_size
        self.mlp_convs = nn.ModuleList()
        ...
        for out_channel in mlp:
            ...
            self.mlp_bns.append(nn.BatchNorm1d(out_channel))
            last_channel = out_channel
        self.linear_p = nn.Sequential(nn.Conv1d(3, 3, 1), nn.BatchNorm1d(3), nn.LeakyReLU(inplace=True), nn.Conv1d(3, po_out_channel, 1))
    ...
    def grid_pool_downsample(self, xyz, target_npoint, return_counts=True):
        ...
        for _ in range(20):  
            ...
            cluster_b = grid_cluster(xyz[0], grid_size_tensor)  # [N]
            unique_clusters, inverse_b = torch.unique(cluster_b, return_inverse=True)  # [N], 0~num_voxels-1
            
            centroids_b = scatter_mean(xyz[0], inverse_b, dim=0)  # [num_voxels, 3]
            total_clusters = centroids_b.shape[0]
        ...
        return result
    def forward(self, xyz1, xyz2, points1, points2,sam_num = None):
        xyz1 = xyz1.permute(0, 2, 1)  # [B, N, 3]
        xyz2 = xyz2.permute(0, 2, 1)  # [B, S, 3]
        B, N, _ = xyz1.shape
        S = sam_num
        results = self.grid_pool_downsample(xyz1,S)
        inverse = results['inverse']
        if points1 is not None:
        ...
        grouped_xyz_list = []
        for b in range(B):
                centers = xyz2[b][inverse[b]]
                points_2_new = points2[0][inverse[b]]
                grouped_xyz = xyz1[b] - centers
                grouped_xyz_list.append(grouped_xyz)
                points2_list.append(points_2_new)
        grouped_xyz = torch.stack(grouped_xyz_list,dim=0)
        points2 = torch.stack(points2_list,dim=0)
        xyz_trans = self.linear_p(grouped_xyz.permute(0,2,1)).permute(0,2,1)
        inverse = (inverse[0].unsqueeze(-1)).unsqueeze(0)
        weight_position = scatter_softmax(xyz_trans, inverse, dim=1)
        new_points = points2 * weight_position
