    ...
    def forward(self, N_Gaussians,g_coords_feac_fusion):
        ...
        for ns in range(self.Ns):
            ...
            for nc in range(self.Nc_m1):
                ...
                x = latent_quant[:,mask_m1_batch[ns]:mask_m1_batch[ns+1],nc*32:(nc+1)*32].squeeze(0)[choose_idx_m1[mask_m1_batch[ns]:mask_m1_batch[ns+1]]]
                likelihood_feac_lat_m1_ctx = 0
                for i in range(3):
                    _, likelihood_feac_lat_m1_com = self.Gaussian_Entropy_Model(x,scale[i],mean[i])
        ...
        feac_m0_scale_quant = feac_m0_scale + torch.empty_like(feac_m0_scale).uniform_(-0.5,0.5)
        ...
        for ns in range(self.Ns):
            ...
            for nc in range(self.Nc_m0):
                ...
                for i in range(3):
                    _, likelihood_feac_lat_m0_com = self.Gaussian_Entropy_Model(x,scale[i]/Q_feac_m0,mean[i]/Q_feac_m0)
        ...
        for ns in range(self.Ns):
            ...
            for i in range(2):
                _, likelihood_geo_lat_com = self.Gaussian_Entropy_Model(x,scale[i]/Q_geo_sp,mean[i]/Q_geo_sp)
