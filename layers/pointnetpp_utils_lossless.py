...
def query_ball_point_knn_aligned(nsample, xyz, new_xyz):
    B, N, _ = xyz.shape
    _, S, _ = new_xyz.shape
    device = xyz.device
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, in_channel, mlp_list, grid_size=None):
        super().__init__()
        ...
        for out_channel in mlp_list:
            self.linear_blocks.append(nn.Linear(last_channel, out_channel))
            last_channel = out_channel
    def forward(self, coords,points):
        """
        Input:
            xyz: [B, C, N] -> [B, N, 3]
            points: [B, D, N] -> [B, N, D]
...
class PointNetSetAbstractionMsg_GridPool(nn.Module):
    def __init__(self,nsample_list, in_channel, mlp_list, npoint = None, grid_size=None):
        super().__init__()
        self.npoint = npoint
        self.nsample_list = nsample_list
    ...
    def forward(self, xyz, points, self_define_samples=None):
        ...
        for i, K in enumerate(self.nsample_list):
            ...
            group_idx = query_ball_point_knn_aligned( K, xyz, new_xyz)  # [B, S, K]
            if group_idx == None:
                return None, None
            ...
            if points is not None:
                gouped_points = index_points(points, group_idx)  # [B, S, K, D]
            else:
                gouped_points = grouped_xyz
            
            # grouped_points = grouped_points.permute(0, 3, 2, 1)  # [B, D+3, K, S]
            for j in range(len(self.linear_blocks)):
                linear = self.linear_blocks[j]
                gouped_points = linear(gouped_points)
                if i < len(self.linear_blocks) - 1:
                    gouped_points = F.leaky_relu(gouped_points)
            gouped_points = torch.max(gouped_points, 2)[0]
            output_list.append(gouped_points)
        
        new_points_concat = torch.cat(output_list, dim=-1)  # [B, D'_total, S]
        return new_xyz, new_points_concat
...
class PointNetFeaturePropagationFast(nn.Module):
    def __init__(self, in_channel, mlp, k=4):
        super().__init__()
        self.k = k
        self.mlp_convs = nn.ModuleList()
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        for i in range(len(self.mlp_convs)):
            linear = self.mlp_convs[i]
            new_points = linear(new_points)
            if i < len(self.mlp_convs) - 1:
                    new_points = F.leaky_relu(new_points)
        return new_points
class PointNetFeaturePropagation(nn.Module):
    def __init__(self, in_channel, mlp, k=8):
        super().__init__()
        self.k = k
        self.mlp_convs = nn.ModuleList()
